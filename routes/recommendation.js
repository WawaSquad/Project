...
function query_db_recommendation(res) {
	  oracle.connect(connectData, function(err, connection) {
	    if ( err ) {
	    	...
	    	var query="SELECT * FROM (SELECT * FROM (("+subquery1+" UNION "+subquery2+" UNION "+subquery3+" UNION "+subquery4+" UNION "+subquery5+" UNION "+subquery6+") MINUS "+subquery7+") ORDER BY dbms_random.value) WHERE ROWNUM<=20"
	    	//console.log(query);
